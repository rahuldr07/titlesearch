 ...
 * is empty look identical, and on a reconciliation screen that is the difference
 * between "the two seats agreed on everything" and "we could not ask".
 *
 * `INVARIANTS:59` is why it wraps a region rather than the screen: "a partial
 * failure degrades that region only." The order's identity keeps rendering
 * while the divergence read is down.
 *
 ...
 * `features/reconciliation` from importing `features/account`, and the two homes
 * that rule leaves — `shared/` and `entities/` — are outside this change's
 * writable set. It is a THIRD copy, counting `account/PanelState`. Stated here rather than laundered through a barrel: the right
...
export function ReadState<T>(props: {
  ...
  /** Names the thing in the sentence — "could not load the divergences". */
  readonly of: string;
