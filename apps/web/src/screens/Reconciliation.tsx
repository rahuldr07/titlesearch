    ...
    <div className="flex h-screen flex-col bg-bg font-sans text-ink">
      <div className="flex flex-none flex-wrap items-baseline justify-between gap-x-[14px] gap-y-2 border-b border-line bg-surface px-[18px] py-[10px]">
        <div className="flex flex-wrap items-baseline gap-[14px]">
          ...
          <div className="text-[12.5px] font-bold text-action tabular-nums">
      ...
      {reconQ.isPending ? (
        ...
        <div className="flex min-h-0 min-w-[1280px] flex-1">
          ...
          <div className="min-w-[560px] flex-1 overflow-y-auto border-l border-line-strong bg-input px-[22px] pt-4 pb-10">
            {ruled.map((d) => (
              <div
                key={d.id}
                data-testid={`ruled-${d.path}`}
                className="mb-[7px] flex flex-wrap items-baseline gap-[10px] rounded-btn border border-line-light bg-surface-dim px-3 py-2 text-[12px]"
              >
                <span className="font-bold text-ok">✓ ruled</span>
                <span className="font-mono">{d.path}</span>
                <span className="text-ink-secondary">
                  {d.ruling_value ?? "Not Available"} · {d.citation}
                </span>
                {d.general_rule_id !== null && (
                  <span className="rounded-chip border border-attend-border bg-attend-bg px-[7px] py-px text-[10px] font-bold text-attend">
            ...
            <div className="mt-4 mb-1 flex flex-wrap items-baseline gap-3">
              <div className="text-[12px] font-bold tracking-[.08em] text-ink-body">
                {open.length} DIVERGENCES OPEN
              </div>
              <span className="rounded-[3px] border border-dashed border-attend-border bg-attend-bg px-2 py-px text-[11px] text-attend">
                Lowest typist accuracy section. Rule with source or not at all.
              </span>
            </div>
            <div className="mb-[10px] text-[11px] text-ink-faint">
...
function DivergenceCard({
  ...
  const sideCard = (side: "A" | "B", value: string | null) => (
    <button
      ...
      className={`cursor-pointer rounded-[5px] border px-3 py-[10px] text-left font-sans ${
        winner === side
          ? "border-action bg-sel-bg shadow-[inset_0_0_0_1px_var(--color-action)]"
  ...
  return (
    <div
      ...
      className="mb-[10px] rounded-card border border-line-strong bg-card px-4 py-[13px] shadow-card"
    >
      ...
      <button
        ...
        className={`mt-2 cursor-pointer rounded-btn border px-3 py-[6px] text-left font-sans text-[12px] ${
          winner === "third"
      ...
      {winner === "third" && (
        <>
          ...
          <input
            ...
            className="mt-[6px] w-full rounded-btn border border-line-strong bg-card px-[10px] py-[7px] font-sans text-[12.5px]"
          />
        </>
      )}
      {winner !== null && (
        <>
          <input
            ...
            className="mt-2 w-full rounded-btn border border-attend-border bg-input px-[10px] py-[7px] font-sans text-[12.5px]"
          />
          {!drafting ? (
            <div className="mt-[10px] border-t border-hairline pt-[10px]">
              ...
              <div className="flex flex-wrap gap-[10px]">
                <button
                  ...
                  className={`rounded-btn border px-[14px] py-[7px] font-sans text-[12px] font-semibold ${
                    baseValid
                      ? "cursor-pointer border-line-strong bg-card text-ink-body"
                      : "cursor-not-allowed border-dashed border-dash bg-track text-ink-dim"
                  }`}
                >
                ...
                <button
                  ...
                  className={`rounded-btn border px-[14px] py-[7px] font-sans text-[12px] font-semibold ${
                    baseValid
                      ? "cursor-pointer border-action-border bg-card text-action"
                      : "cursor-not-allowed border-dashed border-dash bg-track text-ink-dim"
                  }`}
                >
            ...
            <div className="mt-[10px] border-t border-hairline pt-[10px]">
              <div className="mb-[5px] text-[10.5px] font-bold tracking-[.06em] text-action">
              ...
              <textarea
                ...
                className="min-h-[56px] w-full resize-y rounded-[5px] border-[1.5px] border-dash bg-input px-[11px] py-2 font-sans text-[13px] leading-[1.5]"
              />
              <button
                type="button"
                ...
                className={`mt-2 rounded-btn border px-4 py-[7px] font-sans text-[12px] font-semibold ${
