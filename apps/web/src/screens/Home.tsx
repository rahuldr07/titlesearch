...
const toneText: Record<Tone, string> = {
  attend: "font-semibold text-attend",
  act: "font-semibold text-act",
  quiet: "text-ink-dim",
};
    ...
    <ScreenFrame>
      ...
      <div className="flex-1 overflow-y-auto px-[22px] pt-[22px] pb-[60px]">
        <div className="mx-auto max-w-[1014px]" data-testid="home-hub">
          {anyError != null && (
            <p className="mb-3 text-[13px] text-act">
            ...
            {doors.map((d) => {
              ...
              return (
                <Link
                  ...
                  className={`block rounded-card bg-card px-[18px] py-[14px] no-underline ${
                    hot
                      ? sig.tone === "act"
                        ? "border-[1.5px] border-act"
                        : "border-[1.5px] border-attend-border"
                      : "border border-line-mid"
                  }`}
                >
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-[11.5px] font-bold tracking-[.1em] text-label">
                      {d.label}
                    </span>
                    <b className="rounded-chip border border-line-strong bg-line-light px-[5px] font-mono text-[10.5px] text-ink-secondary">
                      g {d.key}
                    </b>
                  </div>
                  {sig !== null && (
                  ...
                  <div className="mt-[5px] text-[11.5px] leading-[1.5] text-ink-dim">
                    {d.desc}
                  </div>
                </Link>
              );
          ...
          <div className="mt-5 text-[12.5px] text-ink-body">
