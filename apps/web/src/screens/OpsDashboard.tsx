      ...
      <ScreenFrame>
        ...
        <div className="mx-auto max-w-[520px] px-[22px] pt-[60px]">
          <div
            ...
            className="rounded-card border border-line-mid bg-card px-7 py-6"
          >
      ...
      <div className="flex-1 overflow-y-auto px-[22px] pt-[18px] pb-[60px]">
        {m && (
          <div className="mx-auto grid max-w-[1520px] grid-cols-[1.55fr_1fr] gap-[14px]">
            ...
            <div
              className={`rounded-card bg-card px-[18px] py-4 ${
                falling ? "border-2 border-act" : "border border-line-mid"
              }`}
            >
              <div className="flex flex-wrap items-baseline justify-between gap-3">
                ...
                {falling && (
                  <span className="rounded-chip bg-act px-2 py-[2px] text-[10.5px] font-bold tracking-[.06em] text-ink-invert">
              ...
              <div className="mt-[10px] flex flex-wrap items-end gap-[22px]">
                <div>
                  ...
                  <div className="mt-2 max-w-[420px] text-[12.5px] leading-[1.5] text-ink-body">
                    Rubber-stamping is automation bias, not a character flaw —
                    and this number is the only place it will ever show up.
                  </div>
                  <div className="mt-[6px] text-[11px] text-ink-dim">
                ...
                {m.catch_rate_history && m.catch_rate_history.length > 0 && (
                  <div className="min-w-[320px] flex-1">
                    <div className="relative flex h-[132px] items-end gap-1 border-b border-line-mid">
                      ...
                      {m.catch_rate_floor != null && (
                        <>
                          <div
                            className="absolute right-0 left-0 border-t-[1.5px] border-dashed border-ink-dim"
                            style={{
                              bottom: `${Math.round((m.catch_rate_floor * 100 - 50) * 2.6)}px`,
                            }}
                          />
                          <div
                            className="absolute right-0 text-[9.5px] tracking-[.05em] text-ink-dim"
                            style={{
            ...
            {/* escalations per 100 */}
            <div className="rounded-card border border-line-mid bg-card px-[18px] py-4">
              <div className="text-[11px] font-bold tracking-[.1em] text-label">
              ...
              {m.escalations_per_100_history &&
                m.escalations_per_100_history.length > 0 && (
                  <>
                    <div className="relative mt-[14px] flex h-[150px] items-end gap-1 border-b border-line-mid">
              ...
              <div className="mt-[10px] text-[12.5px] leading-[1.5] text-ink-body">
              ...
              <div className="mt-[10px] flex items-baseline justify-between border-t border-hairline pt-2">
                ...
                <button
                  type="button"
                  onClick={() => setDrillSignal("escalations")}
                  className="cursor-pointer border-none bg-transparent p-0 text-[11px] font-semibold text-action underline"
                >
                  open clusters →
                </button>
              </div>
            ...
            <div className="rounded-card border border-line-mid bg-card px-[18px] py-4">
              ...
              {(m.correction_by_source ?? []).map((s) => (
                <div
                  ...
                  className="grid cursor-pointer grid-cols-[100px_1fr_74px] items-center gap-3 border-b border-hairline py-[9px]"
                >
                  ...
                  <div>
                    <div className="h-[9px] overflow-hidden rounded-[2px] bg-track">
                      <div
                        className={`h-full ${s.alarm ? "bg-act" : "bg-ink-dim"}`}
                        style={{
              ...
              <div className="mt-2 text-[10.5px] text-ink-dim">
            ...
            <div className="rounded-card border border-line-mid bg-card px-[18px] py-4">
              ...
              <div className="mt-2 text-[11.5px] leading-[1.5] text-ink-body">
                <b>
        ...
        {metricsQ.isPending && (
          <p className="text-[13px] text-ink-dim">Reading the nightly compute…</p>
        )}
        {metricsQ.error != null && (
          <p className="text-[13px] text-act">{String(metricsQ.error)}</p>
        )}
      </div>
      {drillSignal !== null && (
...
function PairCard({
  ...
  return (
    <div className="flex flex-col rounded-card border border-line-mid bg-card px-4 py-[14px]">
      ...
      <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-[10px]">
        <div onClick={onDrill} className="cursor-pointer">
          <div className="text-[10.5px] text-ink-dim">{aLabel}</div>
          <div className="font-mono text-[26px] font-semibold">{aVal}</div>
          <div className="text-[10.5px] text-ink-faint">{aSub}</div>
        </div>
        <div
          className="border-l border-hairline pl-2 text-[9.5px] font-bold tracking-[.06em] text-dash"
          style={{ writingMode: "vertical-rl" }}
        >
          READ TOGETHER
        </div>
        <div onClick={onDrill} className="cursor-pointer">
          <div className="text-[10.5px] text-ink-dim">{bLabel}</div>
          <div
            className={`font-mono text-[26px] font-semibold ${bAct ? "text-act" : ""}`}
          >
            {bVal}
          </div>
          <div className="text-[10.5px] text-ink-faint">{bSub}</div>
        </div>
      </div>
      <div className="mt-[7px] text-[10.5px] leading-[1.45] text-ink-dim">
        {footnote}
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed top-0 right-0 bottom-0 z-[51] flex w-[440px] flex-col border-l border-line-strong bg-input shadow-pop"
      >
        <div className="flex items-center justify-between border-b border-line-light px-4 py-3">
          <div className="text-[12px] font-bold tracking-[.06em] text-ink-body">
            {drillQ.data?.title ?? signal}
          </div>
          <button
            type="button"
            onClick={onClose}
            className="cursor-pointer border-none bg-transparent px-2 py-[2px] text-[16px] text-ink-dim"
          >
            ×
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-3">
          {drillQ.data?.note != null && (
            <div className="mb-[10px] text-[11.5px] leading-[1.5] text-ink-dim">
              {drillQ.data.note}
            </div>
          )}
          {(drillQ.data?.rows ?? []).map((r, i) => (
            <div
              key={i}
              className="mb-[7px] rounded-[5px] border border-line-light bg-card px-3 py-[9px]"
            >
              ...
              {r.meta !== null && (
                <div className="mt-[5px] text-[10.5px] text-ink-faint">
                  {r.meta}
                </div>
              )}
            </div>
