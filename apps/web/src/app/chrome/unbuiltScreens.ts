...
export const UNBUILT_SCREENS: readonly ScreenDescriptor[] = [
  ...
  },
  {
    ...
    missing:
      "BUILT (features/ingest). The gateway checklist and optical profile render inline the moment a file lands; jurisdiction, state and county are read-only, resolved server-side from the recorded clerk stamp; the product select carries CreateOrderRequest.product and the rulebook banner resolves by client and product; the footer is one signed act. Refusals kept: the server's missing-field rejection renders verbatim, and a byte-identical re-upload surfaces the 409 sha256-match notice.",
  },
  {
    path: "/delivery",
    screen: "Delivery",
    ...
    missing:
      "BUILT (features/delivery; the compiler is features/release under /orders/{id}). What this entry used to name as absent arrived with the 2026-08-28 ruling: compile is GET /api/orders/{id}/composition (CompositionResponse with ManifestBlock + GateCheck, design.ts:71-102), sign-and-execute is POST /api/orders/{id}/release with release.compile/release.execute in PERMISSIONS (authz.ts:89-90), and reissue is POST /api/deliveries/{id}/reissue with delivery.reissue (authz.ts:91; design.ts:135-146). Both remaining gaps have since closed and this entry no longer names anything absent: DeliveryStatus is a closed z.enum (enums.ts) carrying draft/signed/digest_recorded/transmitted/acknowledged/failed_transit, so the receipt steps ARE nameable, and Report (entities.ts) now carries `supersedes` and `reason`, so a reloaded version ledger can state why v2 exists without reading the mutation's answer.",
  },
  {
    path: "/escalations",
    screen: "Escalations",
