...
async function getJson<T>(path: string, as?: Role): Promise<T> {
  const res = await fetch(url(path), {
    headers: as === undefined ? {} : { "x-mock-role": as },
  });
...
const postForm = (path: string, form: FormData, as?: Role): Promise<Response> =>
  fetch(url(path), {
    method: "POST",
    headers: as === undefined ? {} : { "x-mock-role": as },
    body: form,
...
async function ingest(bytes: string): Promise<string> {
  const created = await postForm("/api/orders", packageForm(bytes, ORDER_FORM), "ops");
  expect(created.status).toBe(201);
  const { order } = (await created.json()) as { order: { id: string } };
  expect((await post(`/api/orders/${order.id}/accept`, undefined, "admin")).status).toBe(200);
  return order.id;
}
...
async function clearEveryGate(orderId: string): Promise<void> {
  ...
  for (const [id, value] of Object.entries(rulings)) {
    expect((await post(`/api/fields/${fieldId(orderId, id)}/confirm`, { value }, "admin")).status).toBe(
      200,
    );
  }
  // `admin` (→ "L. Vance") ruled these above, so the different-examiner rule
  // requires a distinct seat here: `senior` is the other role `field.countersign`
  // permits, and it resolves to "R. Menon" — the actor is read off the
  // credential (`SEAT_IDENTITIES`), never off a client-supplied header.
  for (const id of ["fld_5", "fld_6"]) {
    const signed = await post(
      `/api/fields/${fieldId(orderId, id)}/countersign`,
      { signature: "R. Menon" },
      "senior",
    );
    expect(signed.status).toBe(200);
...
describe("a known package hydrates the order its upload creates", () => {
  test("the gateway reports the real digest; a known digest resolves the package's own facts, an unknown one keeps today's stamp", async () => {
    ...
    const known = await postForm("/api/intake/quarantine", packageForm(SAMPLE_PACKAGE_BYTES), "ops");
    ...
    const unknown = (await (
      await postForm("/api/intake/quarantine", packageForm(other), "ops")
    ).json()) as Quarantine;
  ...
  test("create fills every per-order read from the bundle, states verbatim, census from the states", async () => {
    ...
    const seedLifecycle = await getJson<{ in_review: { value: number } }>("/api/lifecycle", "ops");

    const created = await postForm("/api/orders", packageForm(SAMPLE_PACKAGE_BYTES, ORDER_FORM), "ops");
    expect(created.status).toBe(201);
    ...
    const id = order.id;
    expect((await post(`/api/orders/${id}/accept`, undefined, "admin")).status).toBe(200);
    ...
    expect(
      (await getJson<{ in_review: { value: number } }>("/api/lifecycle", "ops")).in_review.value,
    ).toBe(seedLifecycle.in_review.value + 1);
    expect((await getJson<{ order: { id: string } | null }>("/api/queue/next")).order?.id).toBe(id);
    const bands = await getJson<{ bands: { id: string; orders: { id: string }[] }[] }>(
      "/api/queue/bands",
      "ops",
    );
    ...
    expect(
      (await post(`/api/fields/${fieldId(id, "fld_5")}/confirm`, { value: "SAMPLETON SAVINGS BANK" }, "senior"))
        .status,
    ).toBe(200);
  ...
  test("a second upload of the same bytes fails de-duplication at the door and 409s on create", async () => {
    ...
    const scan = (await (
      await postForm("/api/intake/quarantine", packageForm(SAMPLE_PACKAGE_BYTES), "ops")
    ).json()) as Quarantine;
    ...
    const again = await postForm("/api/orders", packageForm(SAMPLE_PACKAGE_BYTES, ORDER_FORM), "ops");
  ...
  test("the composition is live over the field states; rulings and second reads open the gates, then release seals", async () => {
    ...
    const refused = await post(`/api/orders/${id}/release`, { signature: "L. Vance" }, "ops");
    ...
    expect(gate(after, "g4")).toMatchObject({ passed: true, detail: "countersigned by R. Menon" });
    ...
    expect((await post(`/api/orders/${id}/release`, {}, "ops")).status).toBe(422);
    const released = await post(`/api/orders/${id}/release`, { signature: "L. Vance" }, "ops");
  ...
  test("a rendered report is the certified artifact — the bundle's report object, verbatim", async () => {
    ...
    await clearEveryGate(id);
    expect((await post(`/api/orders/${id}/release`, { signature: "L. Vance" }, "ops")).status).toBe(200);
  ...
  test("demo reset drops the hydrated order and frees its digest for another upload", async () => {
    ...
    expect(
      (await postForm("/api/orders", packageForm(SAMPLE_PACKAGE_BYTES, ORDER_FORM), "ops")).status,
    ).toBe(201);
  });
});
