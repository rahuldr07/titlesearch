...
export function PageNav({
  ...
  return (
    <div className="flex items-center gap-4 border-b border-line-subtle px-6 py-3">
      ...
      <button type="button" aria-label="Next page" className={step} onClick={() => onStep(1)}>
        ›
      </button>
      <div className="ml-auto flex items-center gap-3">
        <button type="button" aria-label="Zoom out" className={step} onClick={() => onZoom(zoom - 0.1)}>
          −
        </button>
        <span className="font-mono text-xs text-ink-muted">{Math.round(zoom * 100)}%</span>
        <button type="button" aria-label="Zoom in" className={step} onClick={() => onZoom(zoom + 0.1)}>
          +
        </button>
      </div>
    </div>
