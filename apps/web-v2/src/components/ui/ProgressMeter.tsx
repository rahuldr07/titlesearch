...
/**
 * THE DOT METER. Design §Screens 4: "18-dot progress meter + 'N of M decisions
 * settled' (mono)".
 *
 * A dot meter rather than a bar because the quantity is COUNTABLE and small. A
 * bar says "about three quarters"; eighteen dots say "fourteen of eighteen",
 * and on a screen whose whole job is deciding eighteen specific things, the
 * second is the true statement. The reader can also see WHICH ones — a bar
 ...
 *
 * `total` may exceed the dots drawn. Above `MAX_DOTS` the meter draws NO
 * graphic at all and shows the mono count alone. That is deliberate and it is
 * the second-best answer, not the first: a proportional bar needs a computed
 * width, a computed width needs an inline style or an arbitrary value, and
 * `check-rules.mjs` bans both (§6) for reasons that are better than this
 * component's need for a bar. A count with no bar is honest; a bar rounded to
 * the nearest twentieth is a number the screen would then disagree with, which
 * rule 11 exists to prevent. If a long meter is ever genuinely wanted, the
 * answer is a token-driven width class set, not an escape hatch.
 *
 * ACCESSIBILITY: react-aria's ProgressBar supplies role, aria-valuenow/min/max
 * and the label wiring. The dots are decoration on top of a control that is
 * already correct, so a screen reader hears "14 of 18", not eighteen spans.
 */
