...
**Gate 0 outcome (2026-07-22): PORT — the prototype was recovered.** Found in `~/Downloads/titlepipe.zip` (+ `titlepipe_bugfixes.zip`), still outside VCS pending an owner ruling on the real party names in its tests. 2,786 lines across ten modules; `docs/spec.md` recovered with it. The 155 figure = **131 package tests + 24 patch tests**; **145 run green**, and 10 fail only because the county package and five delivered client reports they assert against are correctly absent from VCS. Two findings change the port plan: the five bug fixes are **standalone patches that were never merged** into the package, and `v14` **does not exist**. Full evidence: `docs/backend/GATE_0_RECOVERY.md`.
